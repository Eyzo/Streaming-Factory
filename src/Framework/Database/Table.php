...
class Table
{
    ...
    public function deletePostGenre(int $id_post,int $id_genre):bool
    {
        $query = $this->pdo->prepare("DELETE FROM {$this->table} WHERE id_post = :id_post AND id_genre = :id_genre");
        $query->bindValue(':id_post',$id_post);
        $query->bindValue(':id_genre',$id_genre);
